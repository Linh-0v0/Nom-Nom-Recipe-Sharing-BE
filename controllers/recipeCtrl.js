  ...
  user_auth(req, res, async () => {
    ...
    try {
      const insertRecipeQuery = `
        ...
        RETURNING recipe_id
      `;
      const insertRecipeValues = [userId, name, servingSize, duration, imageLink, description];
      const insertRecipeResult = await db.query(insertRecipeQuery, insertRecipeValues);
...
recipeCtrl.get = async (req, res) => {
  ...
  try {
    const recipe = await db.one('SELECT * FROM recipe WHERE recipe_id = $1', [id]);
...
recipeCtrl.updateRecipe = async (req, res) => {
  ...
  try {
    ...
    // Update the recipe
    const updateRecipeQuery = 'UPDATE recipe SET name = $1, serving_size = $2, duration = $3, image_link = $4, description = $5, updated_at = NOW() WHERE recipe_id = $6';
    const updateRecipeValues = [name, serving_size, duration, image_link, description, recipeId];
...
recipeCtrl.deleteRecipe = async (req, res) => {
  ...
  try {
    ...
    const selectRecipeQuery = 'SELECT author_id FROM recipe WHERE recipe_id = $1';
    const selectRecipeValues = [recipeId];
    const selectRecipeResult = await client.query(selectRecipeQuery, selectRecipeValues);
    ...
    const deleteCollectionRecipeQuery = 'DELETE FROM collection_recipe WHERE recipe_id = $1';
